...
# Assume 'business_data' has been read from your data source as before
...
top_business_types <- business_trends %>%
  ...
  arrange(desc(Total_Count)) %>%  # Sort in descending order of Total_Count
  slice(2:11) %>%  # Select the rows corresponding to ranks 2 to 11
  pull(Business_Type)  # Extract just the Business_Type column
# Convert Business_Type to a factor with levels ordered by Total_Count
...
# Your previous data processing steps here...
...
plot <- ggplot(filtered_trends, aes(x = Year, y = Count, color = Business_Type)) +
  ...
  theme_minimal() +
  theme(
    axis.text.x = element_text(angle = 90, hjust = 1, size = 8), # Set size for axis text
    legend.position = "right", # Position the legend to the right
    legend.text = element_text(size = 6), # Set smaller size for legend text
    legend.title = element_text(size = 8), # Set smaller size for legend title
    legend.key.size = unit(0.5, "cm"), # Set smaller size for legend keys
    legend.margin = margin(t = 0, r = 0, b = 0, l = 0), # Remove margin around legend
    plot.margin = margin(t = 1, r = 1, b = 1, l = 1, unit = "cm") # Adjust plot margins if needed
  ) +
  guides(color = guide_legend(title = "Business Type")) # Add a legend for business types
